...
void internalAppendStu(student data[], int dataCount)
{
    ...
    if (data[dataCount].name[0] >= 97 && data[dataCount].name[0] <= 122)
        data[dataCount].name[0] -= 32;
L7:
...
void dummyStudent(student data[], int *dataCount)
{
    strcpy(data[*dataCount].name, "Druba");
    data[*dataCount].rollno = 1;
    data[*dataCount].phNo = 100;
    (*dataCount)++;
    strcpy(data[*dataCount].name, "ABC");
    data[*dataCount].rollno = 2;
    data[*dataCount].phNo = 101;
    (*dataCount)++;
    strcpy(data[*dataCount].name, "NUL");
    data[*dataCount].rollno = 3;
    data[*dataCount].phNo = 102;
    (*dataCount)++;
    strcpy(data[*dataCount].name, "NULISMADHA");
    data[*dataCount].rollno = 4;
    data[*dataCount].phNo = 102;
    (*dataCount)++;
}
...
void d_program()
{
    ...
    int stDataCount = 0;
    int option = 0;
    dummyStudent(stData, &stDataCount);
L1:
    system("clear");
